...
class Executor:
    ...
    async def run(self, query: str, *, session_id: str | None = None,
        ...
        # Auto-generate HTML replay viewer
        try:
            from replay_viewer import generate_html
            from pathlib import Path
            replay_path = Path(__file__).parent / "state" / "sessions" / sid / "replay.html"
            replay_path.write_text(generate_html(sid))
            print(f"📄 Replay saved: {replay_path.resolve()}")
        except Exception as e:
            print(f"[flow] replay generation skipped: {e}")
